...
def get_armor_defaults(
    conn: sqlite3.Connection, armor_name: str
) -> ArmorSearchDefaults:
...
def set_armor_defaults(
    conn: sqlite3.Connection, armor_name: str, defaults: ArmorSearchDefaults
) -> None:
    """指定した防具の検索初期設定を保存する。"""
...
def list_armors_using_skill_set(
    conn: sqlite3.Connection, skill_set_name: str
) -> list[str]:
